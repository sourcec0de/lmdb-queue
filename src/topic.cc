...
Topic::~Topic() { mdb_dbi_close(_env->getMdbEnv(), _desc); }

bool checkConsumerKeyPrefix(const MDB_val& val) {
  const char* str = (const char*)val.mv_data;
  return val.mv_size > strlen(prefixConsumerStr) &&
      strncmp(str, prefixConsumerStr, strlen(prefixConsumerStr)) == 0;
}
...
TopicStatus Topic::status() {
  ...
  while (rc == 0 && checkConsumerKeyPrefix(cur.key())) {
    ...
    const char* namePtr =
        ((const char*)cur.key().mv_data) + strlen(prefixConsumerStr);
...
void Topic::setProducerHeadFile(Txn& txn, uint32_t file, uint64_t offset) {
  MDB_val key{sizeof(file), &file}, val{sizeof(offset), &offset};

  mdb_put(txn.getEnvTxn(), _desc, &key, &val, 0);
}

uint64_t Topic::getProducerHead(Txn& txn) {
  MDB_val key{strlen(keyProducerStr), (void *)keyProducerStr}, val{0, 0};
...
uint64_t Topic::getConsumerHead(Txn& txn, const std::string& name) {
  ...
  snprintf(keyStr, strlen(keyConsumerStr) + strlen(name.c_str()) + 1,
           keyConsumerStr, name.c_str());
...
void Topic::setConsumerHead(Txn& txn, const std::string& name, uint64_t head) {
  ...
  MDB_val key{strlen(keyStr), keyStr}, val{sizeof(head), &head};

  mdb_put(txn.getEnvTxn(), _desc, &key, &val, 0);
}

int Topic::getChunkFilePath(char* buf, uint32_t chunkSeq) {
  return snprintf(buf, 4096, "%s/%s.%d", getEnv()->getRoot().c_str(),
                  getName().c_str(), chunkSeq);
}
